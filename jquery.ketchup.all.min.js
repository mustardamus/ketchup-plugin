...
g(this);h.bindValidationEvent(b,j).callInitFunctions(b,j)});b.data(this.dataNames.elements,c);this.bindFormSubmit(b)},initFunctions:function(){var b=this.options,c=["createErrorContainer","showErrorContainer","hideErrorContainer","addErrorMessages"];for(f=0;f<c.length;f++){var d=c[f];b[d]||(b[d]=this[d])}return this},initFields:function(b,c){var d=this,h=this.dataNames,j=g(!c?this.fieldsFromForm(b):this.fieldsFromObject(b,c));j.each(function(){var k=g(this),m=d.extractValidations(k.data(h.validationString),
d.options.validateIndicator);k.data(h.validations,m)});return j},callInitFunctions:function(b,c){var d=c.data(this.dataNames.validations);for(i=0;i<d.length;i++)d[i].init.apply(this.helpers,[b,c])},fieldsFromForm:function(b){var c=this,d=this.options,h=this.dataNames,j=d.validateElements,k=[];j=typeof j=="string"?[j]:j;for(i=0;i<j.length;i++){var m=b.find(j[i]+"["+d.attribute+"*="+d.validateIndicator+"]");m.each(function(){var l=g(this),p=l.attr(d.attribute),q=c.extractEvents(p,d.eventIndicator);
l.data(h.validationString,p).data(h.events,q?q:d.validateEvents)});k.push(m.get())}return this.normalizeArray(k)},fieldsFromObject:function(b,c){var d=this.options,h=this.dataNames,j=[];for(s in c){var k,m;if(typeof c[s]=="string"){k=c[s];m=d.validateEvents}else{k=c[s][0];m=c[s][1]}var l=b.find(s);k=this.mergeValidationString(l,k);m=this.mergeEventsString(l,m);l.data(h.validationString,d.validateIndicator+"("+k+")").data(h.events,m);j.push(l.get())}return this.normalizeArray(j)},mergeEventsString:function(b,
c){var d=b.data(this.dataNames.events),h="";if(d){d=d.split(" ");for(i=0;i<d.length;i++)if(c.indexOf(d[i])==-1)h+=" "+d[i]}return g.trim(c+h)},mergeValidationString:function(b,c){var d=this.options,h=b.data(this.dataNames.validationString),j=function(l){var p=l.name;if(l.arguments.length)p=p+"("+l.arguments.join(",")+")";return p},k=function(l,p){for(i=0;i<l.length;i++)if(l[i].name==p.name)return true};if(h){var m=this.extractValidations(d.validateIndicator+"("+c+")",d.validateIndicator);d=this.extractValidations(h,
d.validateIndicator);c="";for(o=0;o<d.length;o++)c+=j(d[o])+",";for(n=0;n<m.length;n++)k(d,m[n])||(c+=j(m[n])+",")}return c},bindFormSubmit:function(b){var c=this;b.submit(function(){return c.allFieldsValid(b,true)})},allFieldsValid:function(b,c){var d=this,h=true;b.data(this.dataNames.elements).each(function(){var j=g(this);if(d.validateElement(j,b)!=true){c==true&&d.triggerValidationEvents(j);h=false}});b.trigger("formIs"+(h?"Valid":"Invalid"),[b]);return h},bindValidationEvent:function(b,c){var d=
this,h=this.options,j=this.dataNames,k=c.data(j.events).split(" ");for(i=0;i<k.length;i++){c.bind("ketchup."+k[i],function(){var m=d.validateElement(c,b),l=c.data(j.container);if(m!=true){if(!l){l=h.createErrorContainer(b,c);c.data(j.container,l)}h.addErrorMessages(b,c,l,m);h.showErrorContainer(b,c,l)}else h.hideErrorContainer(b,c,l)});this.bindValidationEventBridge(c,k[i])}return this},bindValidationEventBridge:function(b,c){b.bind(c,function(){b.trigger("ketchup."+c)})},validateElement:function(b,
...
b.data(this.dataNames.events).split(" "),d=0;d<c.length;d++)b.trigger("ketchup."+c[d])},extractValidations:function(b,c){for(var d=b.substr(b.indexOf(c)+c.length+1),h="",j=[],k=0,m=[],l=0;l<d.length;l++)switch(d[l]){case "(":h+="(";k++;break;case ")":if(k){h+=")";k--}else j.push(g.trim(h));break;case ",":if(k)h+=",";else{j.push(g.trim(h));h=""}break;default:h+=d[l]}for(v=0;v<j.length;v++){k=j[v].indexOf("(");d=j[v];h=[];if(k!=-1){d=g.trim(j[v].substr(0,k));h=g.map(j[v].substr(d.length).split(","),
function(p){return g.trim(p.replace("(","").replace(")",""))})}if((k=this.validations[d])&&k.message){l=k.message;for(a=1;a<=h.length;a++)l=l.replace("{arg"+a+"}",h[a-1]);m.push({name:d,arguments:h,func:k.func,message:l,init:k.init})}}return m},extractEvents:function(b,c){var d=false,h=b.indexOf(c+"(");if(h!=-1)d=b.substr(h+c.length+1).split(")")[0];return d},normalizeArray:function(b){var c=[];for(i=0;i<b.length;i++)for(e=0;e<b[i].length;e++)b[i][e]&&c.push(b[i][e]);return c},createErrorContainer:function(b,
